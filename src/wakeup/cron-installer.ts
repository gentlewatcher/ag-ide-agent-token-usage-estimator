...
import { 
  ...
  isWindowsTaskInstalled, 
  getWindowsTaskStatus 
} from './windows-scheduler.js'

/**
 * Check if running on a platform supporting Unix cron (macOS or Linux)
 */
...
/**
 * Install cron/scheduled job for scheduled wake-up
...
export async function installCronJob(cronExpression: string): Promise<CronInstallResult> {
  ...
  if (!isCronSupported()) {
    return {
      success: false,
      error: `Scheduler is not supported on ${process.platform}.`,
...
export async function uninstallCronJob(): Promise<boolean> {
  ...
  if (!isCronSupported()) {
    debug('cron-installer', 'Scheduler not supported on this platform')
    return false
  }
...
/**
 * Get current scheduled job status
 */

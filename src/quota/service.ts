...
export async function fetchQuota(method: QuotaMethod = 'auto'): Promise<QuotaSnapshot> {
  if (method === 'auto') {
    try {
      ...
      debug('service', 'Auto mode: local method failed, trying Google Cloud Code API', err)
      // Fallback to Google if user is logged in (including via native ~/.gemini tokens)

...
### `antigravity-usage agent` 🤖
Track Antigravity Agent token usage, session statistics, and tool calls directly from your local conversation transcripts.

```bash
antigravity-usage agent             # View overall summary of agent token analytics
antigravity-usage agent sessions    # List recent agent conversation sessions
antigravity-usage agent session <id># Inspect step-by-step token details for a conversation
antigravity-usage agent models      # View breakdown of token usage by model
antigravity-usage agent --json      # Export agent analytics as JSON
antigravity-usage agent --since 7d  # Filter activity to the last 7 days
```

### `antigravity-usage estimate` 🔮
Predict estimated token consumption and tool executions for unsubmitted prompts or pending implementation plans before approval, with cross-model statistical scaling and status bar badge payloads for IDE extensions.

```bash
antigravity-usage estimate --prompt "Add user authentication"   # Estimate from draft prompt
antigravity-usage estimate --plan path/to/implementation_plan.md# Estimate from proposed plan
antigravity-usage estimate --model gemini-3.7-pro               # Scale prediction for a target model
antigravity-usage estimate ratios                               # View learned cross-model statistical ratio matrix
antigravity-usage estimate --badge                              # Output bottom status bar badge text
antigravity-usage estimate --json                               # Export full status bar payload as JSON
antigravity-usage estimate --sync                               # Sync and recalibrate from local brain traces
```

### `antigravity-usage settings` ⚙️
Open configuration in your active IDE editor with offline JSON Schema autocomplete, or launch the interactive glassmorphic web settings dashboard.

```bash
antigravity-usage settings --open          # Open settings.json directly in active IDE editor
antigravity-usage settings --web           # Launch local web settings dashboard in browser
antigravity-usage settings --port 3840     # Custom port for dashboard server
antigravity-usage settings --json          # Print settings as JSON
```

### `antigravity-usage config` 🔧
Inspect or modify individual configuration key-value pairs from the terminal.

```bash
antigravity-usage config list              # List all settings & descriptions
antigravity-usage config get defaultModel  # Get current value for a setting
antigravity-usage config set defaultModel gemini-3.7-pro # Update setting
antigravity-usage config open              # Open settings.json in editor tab
antigravity-usage config reset             # Reset all settings to defaults
```

### `antigravity-usage menu` 🎯
Launch the interactive bottom bar QuickPick command dispatcher or export context menu definitions for IDE integration.

```bash
antigravity-usage menu                     # Interactive terminal QuickPick menu
antigravity-usage menu --json              # Export status bar right-click context menu JSON
antigravity-usage menu --execute estimate-plan # Directly execute an action ID
```
...
Troubleshoot issues with your setup. Checks env vars, auth status, Antigravity brain discovery, and local server connectivity.
...
> **Platform Support:** Fully supported on **Windows** (via native PowerShell Windows Task Scheduler), **macOS**, and **Linux** (via system cron).

```bash
antigravity-usage wakeup config     # Interactive setup (takes 30 seconds)
antigravity-usage wakeup install    # Install to system scheduler (Windows Task Scheduler / Cron)
